...
Component({
  ...
  attached() {
    console.log(this.properties.imgSrc,'临时路径');
    console.log(this.properties.height2)
    this.setData({
      height: this.properties.height2,
      width: this.properties.width2

    })
    this.pushImg(this.properties.imgSrc);
        ...
        let y = event.detail ? event.detail.y : event.touches[0].clientY;
          ...
          wx.canvasToTempFilePath({
            ...
            success: (res) => {
              console.log(res)
              wx.uploadFile({
                url: api + 'manage-api/resource/cardScan/uploadImage',
                filePath: res.tempFilePath,
                name: 'file',
                header: { 'Content-Type': 'application/json' },
                success(res2) {
                  wx.hideLoading()
                  var imgUrl = JSON.parse(res2.data);
                  var p1 = imgUrl.url.slice(imgUrl.url.indexOf('.com') + 4, imgUrl.url.length);
                  var p2 = imgUrl.cjurl ? imgUrl.cjurl.slice(imgUrl.cjurl.indexOf('.com') + 4, imgUrl.cjurl.length):p1;
                  _this.triggerEvent('tapcut', {
                    lsUrl: res.tempFilePath,
                    url: p1,
                    width: _this.data.width * _this.data.export_scale,
                    height: _this.data.height * _this.data.export_scale,
                    photoCj: p2,
                  });
                }
              })   
