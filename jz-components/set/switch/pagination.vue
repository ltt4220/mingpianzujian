<template>
  <nav class="zm-pageNav">
    ...
    <ul class="pagination">
      ...
      <li
        :class="[{'disabled': current == page},'buttonPage']"
        @click="setCurrent(current + 1)"
      >
    ...
    <ul class="pagination pull-right">
      ...
      <li>
        <button
          @click="setCurrent(showPage)"
          class="btnCheck"
        >确定</button>
      </li>
    </ul>
    <span v-html="pageStyle"></span>
  </nav>
</template>
...
export default {
  ...
  created() {
    this.isHover == 0 ? (this.isZmAbck = true) : (this.isZmAbck = false);
  },
  computed: {
    page: function() {
      // 总页数
