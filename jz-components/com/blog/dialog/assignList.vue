  ...
  <div class="zm-dialog-pageBox">
    <EasyScrollbar :barOption="myBarOption">
      <ul class="zm-dialog-pageList">
        ...
        <li
          v-for="(ite,i) in setBlogList"
          :key="i"
          class="zm-dialog-pageList-one sliderDragLi clearFloat"
          :data-fid="ite.fId"
          :data-show="true"
          :data-time="ite.fCreateTime"
          v-bind:class="{'onlay':i===isOnlay,'wrong':i===isWrong}"
          v-dragging="{ item: ite, list: lists, group: 'ite'}"
          @mouseenter="addMouseEnter(i)"
          @mouseleave.stop="addMouseLeave(i)"
        ...
        >
          ...
          <div class="zm-dialog-Title">
            <input
              ...
              @dragstart="addDragstart($event)"
              @dragenter="dragenter(ite)"
              readonly
            >
            <i
              class="zm-boxShow"
          ...
          <span
            class="zm-dialog-eye iconfont"
            v-bind:class="{'icon-eyeOn': ite.isIconEyeOn && !ite.isIconDuigou1}"
            @click.self.prevent="addHidShow(i)"
          ></span>
          <span
            class="iconfont zm-eyeoff"
            v-bind:class="{'icon-eyeOff':!ite.isIconEyeOn && !ite.isIconDuigou1}"
            @click.self.prevent="addIconEyeOff(i)"
          ></span>
          <span
            class="zm-dialog-setting iconfont"
            v-bind:class="{'icon-delete':ite.iconShezhi2}"
            @click.stop.prevent="addReMove(ite,i)"
          ></span>
          <p
            class="zm-dialog-blogNameError iconfont icon-jinzhitishi"
            v-if="i===isJiZhi"
            @mouseenter.self.prevent="addJiZhiEnter"
            @mouseleave.self.prevent="addJiZhiLeave"
          ></p>
          <p
            class="zm-dialog-blogReNameError"
            v-if="i===isDubeName && isDUbeShow"
          >{{errorTitle}}</p>
          <p
            class="zm-dialog-blogNameErrorText"
            v-if="i===isNameOver && isNameShow"
          >{{errorTitle}}</p>
        </li>
      </ul>
    </EasyScrollbar>
    <div
      class="zm-dialog-carousel-pageSetBox"
      @mouseenter="pageSetEnter()"
      @mouseleave="pageSetLeave()"
      v-if="isSelect"
      :style="{top:objCss.top,left:objCss.left}"
    >
...
export default {
  ...
  computed: {
    setBlogList() {
      this.lists = zmEditor.$store.state.component.selectList[0].lists; //blog组件数据;
      return this.lists; //blog组件数据;
    },
    checkId() {
  ...
  methods: {
    ...
    addEyeInit(index) {
      ...
      console.log(_this.lists, "___+===================-444444444");
