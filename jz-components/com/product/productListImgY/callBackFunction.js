...
export default {
    methods: {
        ...
        setMobHeight(val) { //计算mobile高度;
            ...
            let cont; //mobile高度;
            this.settingData.pageShow.value ? (pageHeight = pageTop + pageTitHeight) : pageHeight = 0; //区分显示隐藏分页的高度;
            cont = parseFloat(imgTop + imgHeight + titTop + titHeight + priceTop + priceHeight + priceBottom + detailsHeight + detailsBottom) * boxNum + (boxBottom * (boxNum - 1)) + pageHeight; //产品总高度包括分页;
        ...
        initMobile() {
            ...
            if (set.mobNum.flag) {
                ...
                let num;
                num = rows * cols;
                num > 8 ? set.mobNum.value = 8 : set.mobNum.value = num;
            }
        ...
        setPcHeight(val, boen = false, drag = false) { //pc产品高度计算;
            ...
            let detailsHeight = this.settingData.skuShow.value ? parseInt(this.settingData.skuHeight.value) : 0; //查看详情高度;
            let detailsBottom = this.settingData.skuShow.value ? parseInt(this.settingData.skuMarginBottom.value) : 0; //查看详情的下边距;
            ...
            this.settingData.titleDBHeight.value.text == "单行文本" ? titHeight = parseFloat(titHeight * titleFontSize) : ""; //区分单双行标题的字体高度;
            this.settingData.pageShow.value ? (pageHeight = pageTitHeight + pageTop + pageBottom) : pageHeight = 0; //区分显示隐藏分页的高度;
            let cont;
            cont = parseFloat(parseFloat(imgHeight + titTop + titHeight + priceTop + priceHeight + priceBottom + detailsHeight + detailsBottom) * rowsVal) + (rowsVal - 1) * boxBottom + pageHeight; //产品总高度包括分页;
